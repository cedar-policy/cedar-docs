...
+ **[Effect](#term-policy-effect)** –The effect specifies the *intent* of the policy, to either `permit` or `forbid` any request that matches the scope and conditions specified in the policy\.
+ **[Scope](#term-policy-scope)** – The scope specifies the combination of principals, actions, and resources to which the policy applies\. Inclusion of these elements is mandatory\. A policy that has a *only* scope without additional context conditions can be part of a [role\-based access control](https://wikipedia.org/wiki/Role-based_access_control) strategy\.
+ **[Conditions](#term-parc-context)** – *\(optional\)* You can optionally provide additional conditions\. These conditions must be satisfied for the policy to affect the evaluation of the authorization request\. These conditions are expressed as `when` and `unless` clauses\. You can use them to evaluate the attributes of the principals, resources, and other elements that make up the context of the request\. A policy that includes conditions in a policy can be part of an [attribute\-based access control](https://wikipedia.org/wiki/Attribute-based_access_control) strategy\.
...
+ `permit` – If all elements in the scope match, all `when` conditions evaluate to `true` and all `unless` conditions evaluate to `false`, then the policy results in an `Allow`\.
+ `forbid` – If all elements in the scope match, all `when` conditions evaluate to `true` and all `unless` conditions evaluate to `false`, then the policy results in a `Deny`\.
...
These are two key principles to remember that embody the previous two rules:
...
+ A `Deny` result in for any policy evaluation results in an overall `Deny` for the request\. This is referred to as an *explicit deny*\.
**Important**  
An explicit `Deny` in any one policy ***always ***overrides any `Allow` from other policies\.

### Examples<a name="term-policy-effect-examples"></a>
...
A request always includes information that Cedar uses to answer these three questions:
...
The `principal` element in a Cedar policy represents a role, user, service, or other identity that can make a request to perform an `action` on a `resource` in your app\. If the principal making the request matches the `principal` defined in this policy statement, then this element matches\.

The `principal` element must be present\. If you specify only `principal` without an express that constrains its scope, then the policy applies to *any* principal\.

#### Examples<a name="term-parc-principal-examples"></a>
...
The `action` element in a Cedar policy is a list the operations in your app for which this policy statement controls access\. If the operation in the request matches the one of the `action` items defined in this policy statement, then this element matches\.

#### Examples<a name="term-parc-action-examples"></a>
...
**Resource** – a resource defined by your app that can be accessed or modified by the specified action\. 

The `resource` element must be present\. If you specify only `resource` without an express that constrains its scope, then the policy applies to *any* resource\.

All of these, `principal`, `action`, and `resource`, are defined as entities\.

#### Examples<a name="term-parc-resource-examples"></a>
...
In comparison to the scope values of `principal`, `action`, and `resource`, which represent stable information in the system and must be present in every request, `context` represents information about a point\-in\-time request and is required only for relevant request scenarios\.
...
#### Examples<a name="term-parc-context-when-examples"></a>

The following `when` example matches the request if the principal making the request is a member of the `HardwareEngineering` department and has a `jobLevel` of at least `5`\. Note that if either attribute is missing from the principal making the request, then the policy doesn't match\.
...
#### Examples<a name="term-parc-context-unless-examples"></a>
...
You can attach arbitrary string values to Cedar policies in the form of annotation\. An annotation has no impact on policy evaluation\. They are stored with the policy, and are available for use by services that read and process Cedar policies\.
...
For example, the following shows two annotations that could be part of a policy\.
