...
Cedar is the policy language used by [several AWS authorization services](#related-services). They take the separation of business logic and authorization logic one step further: The services host Cedar policies, and provides APIs for managing them and carrying out authorization decisions on behalf of all your applications. They handle the heavy lifting so your applications don't have to. 
...
+ Although the evaluation of any Cedar policy is guaranteed to terminate, a malicious user could attempt to submit a very lengthy policy, which could incur either storage or performance costs. If you are evaluating arbitrary Cedar policies submitted by users, we recommend enforcing a length limit on them.
