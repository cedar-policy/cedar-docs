...
By default, it is entirely up to the application to make sure that authorization requests are well-formed according to the schema's expectations. However, Cedar provides utilities to _optionally_ validate a _PARC_ request adheres to the expectations given in the schema. The following are the currently available validation utilities:

+ [Cedar CLI](https://github.com/cedar-policy/cedar/tree/main/cedar-policy-cli)
+ [Request::new()](https://docs.rs/cedar-policy/latest/cedar_policy/struct.Request.html#method.new) in the Rust API
