...
+ **An entity type** &ndash; The type determines which attributes are required or supported for entities of that type. Examples include things like `User`, `Photo`, `Album`, `Group`, or `Account`. Define entity types as part of your application's [schema](../overview/terminology.html#term-schema). The schema assigns special meaning to the `Action` entity type, only allowing it to be used as the type of the `action` variable in an authorization request, not disallowing all attributes and tags on actions entities.  The Cedar language reserves the `__cedar` entity type, so this entity type, and all entity types with `__cedar` as a component, cannot be used.
